...
namespace Proje
{
    ...
    {
        private string kAdi;//Kullanıcı adı parametresi ile otomasyonun kontrolü sağlanıyor.

        //SQL Bağlantısı
        string baglantiString = "Data Source=SONGUL;Initial Catalog=proje;Integrated Security=True;";
        //Seçilen ürünlerin toplam tutarını parasal olarak hesaplamak için değişken tanımlanıyor.
        private decimal toplamTutar = 0;
        //Seçilen ürünlerin tutulduğu kütüphane yapısı.
        ...
        private void menu_Load(object sender, EventArgs e)
        {
            ...
            yiyecekleriYukle(); //yiyecekleriYükle fonk.
            butonlariBagla(); //butonlariBagla fonk.
        }

        private void butonlariBagla()
        {
            //Menüdeki numericupdownların çalışamsını sağlar her bir arttırma ve azaltma butonları ile ilişkili yiyecek değeri arttırılır.
            var buttonsAndNumericUpDowns = new (Button btnArtir, Button btnAzalt, NumericUpDown numUpDown)[]
            {
                //butonları ve numericupdownu eşleştiriyoruz. Hangi arttır ve azalat butonu hangi numericupdown'u etkilemeli?
             ...
             (btn_arttır25, btn_azalt25, numericUpDown25), (btn_arttır26, btn_azalt26, numericUpDown26),(btn_arttır27, btn_azalt27, numericUpDown27),
            ...
            foreach (var (btnArtir, btnAzalt, numUpDown) in buttonsAndNumericUpDowns)
            {
                ...
                btnArtir.Click += btnArtirAzalt_Click; //arttırma olayını çalıştır
                btnAzalt.Click += btnArtirAzalt_Click; //azaltma olayını çalıştır.
            }
        }
        private void btnArtirAzalt_Click(object sender, EventArgs e)
        {
            ...
            if (btn != null && btn.Tag is NumericUpDown)
            {
                ...
                if (btn.Text == "+" && numUpDown.Value < numUpDown.Maximum) //eğer butonun text özelliği + ise numericUpdownın değerini 1 arttır
                {
                    numUpDown.Value += 1;
                }
                else if (btn.Text == "-" && numUpDown.Value > numUpDown.Minimum) //eğer butonun text özelliği - ise numericUpdownın değerini 1 azalt
                {
                    numUpDown.Value -= 1;
                }
            }
        ...
        {
            string sorgu = "SELECT YiyecekAdı, Resim, Fiyat FROM Yemekler";//Yemekler tablosundaki YiyecekAd, Resim ve Fiyat bilgilerini al
            //sql bağlantısı
            using (SqlConnection baglanti = new SqlConnection(baglantiString))
            using (SqlCommand cmd = new SqlCommand(sorgu, baglanti))
            {
                try //hata verebilecek kodlar bu kısımdadır.
                {
                    baglanti.Open();//bağlantıyı aç
                    using (SqlDataReader reader = cmd.ExecuteReader()) //SQL'i oku
                    {
                        Dictionary<string, (Label, PictureBox, TextBox)> yemekler = new() //Yemekler için picturebox label ve textbox değerlerinin tutulduğu bir sözlük tanımlanır.
                        {
                            //sqlde Kahvaltı Tabağı adı aranıyor bulunursa label'a ismi, picturebox'a resim ve textbox'a da fiyat bilgileri yerleştiriliyor.
                            //Eğer ilk veriyi sqlde bulamazsa isim, resim ve fiyat verileri görüntülenemiyor.
                            //Kahvaltılar
                        ...
                        }//tüm picturebox'ın size özelliğinin aynı yapma.

                        while (reader.Read()) //yemek adlarını resimleri ve fiyatlarını sqlden okuyarak tasarımdakiyle eşleştirir ve ekrana yansıtır.
                        {
        ...
        private void SepeteEkle(string urunAdi, NumericUpDown numericUpDown)
        {
            ...
            if (adet <= 0) //Eğer seçilen adet 0'dan küçükse bu mesajı göster.
            {
            ...
            try
            {
                ...
                {
                    con.Open(); //bağlantıyı çalıştır.
                    string fiyatQuery = "SELECT Fiyat FROM Yemekler WHERE YiyecekAdı = @urun";
                    //Yemekler tablosundaki yiyecekAdınına göre fiyat bilgisi alınır decimal türüne dönüşütürlür.
                    ...
                    // Adet sıfır değil ve fiyat bulunduysa tutarı hesapla
                    ...
                    // Sepete eklenilen ürünün bilgisini göster
        ...
        //Her bir Sepete ekle butonu sepete ekle netoduna gönderilir. Ad ve Fiyat bilgileri ile.
        ...
        private void btn_sepet_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(baglantiString))
                {
                    ...
                    foreach (var urun in secilenUrunler) //secilenurunler dizisinde ürün varlığını kontrol eder.
                    {
                        string urunAdi = urun.Key; //ürün varsa adını ve adetini alır.
                        int adet = urun.Value;

                        if (adet <= 0) //adeti 0den küçük ya da eşitse fonksiyona devam edilir.
                            continue;

                        string fiyatQuery = "SELECT Fiyat FROM Yemekler WHERE YiyecekAdı = @urunAdi";
                        //Ürün adlarına göre fiyatları alınır.
                        using (SqlCommand fiyatCmd = new SqlCommand(fiyatQuery, con))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertQuery, con))
                            {
                                ...
                                cmd.ExecuteNonQuery();
                                enAzBirEklendi = true;
                                eklemedenAtlama = false;
                            }
                        }
                    }
                }

                if (enAzBirEklendi || eklemedenAtlama)
                {
